...
const StepForm = ({ id }) => {
  ...
  const [structureOptions, setStructureOptions] = useState([]); // 定义 productOptions
  ...
  const steps = [
    ...
    },
    {
      ...
      handleSubmit: async () => {
        try {
          const saveData = Array.isArray(formKitData) ? formKitData : Array.isArray(formKitData.data) ? formKitData.data : [];
    ...
    },
    {
      ...
      <StructureDimensions
        ...
        setFormStructureData={(data) => setFormStructureData({data })}
        productOptions={structureOptions}  // 传递 productOptions
      ...
      loadData: async () => {
        if (projectId) {
          ...
          const response = await fetchProjectFieldsByProjectIdParentId(projectId,singledata.data.id);
          setFormStructureData(response.data);
         }
      },
      handleSubmit: async () => {
        try {
          const saveData = Array.isArray(formStructureData) ? formStructureData : Array.isArray(formStructureData.data) ? formStructureData.data : [];
        if (saveData.length === 0) {
            console.warn('saveProjectFieldData received an empty array, skipping API call.');
          } else {
            await saveProjectFieldData(saveData);
