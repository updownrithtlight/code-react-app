...
const InputForm = ({ projectId, fieldId }) => {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Form.Item name={["parameters", record.key, "unit"]} style={{ margin: 0 }}>
          ...
          >
            {record.parameter === "输入电压" || 
             record.parameter === "输入过压浪涌" ||
             record.parameter === "输入尖峰电压" ||
             record.parameter === "输入欠压浪涌" ||
             record.parameter === "输入防反接保护" ||
             record.parameter === "输入电压中断" 
              ? (
              <>
                <Option value="Vdc">Vdc</Option>
                <Option value="Vac">Vac</Option>
              </>
            ) : record.parameter === "输入电流抑制"  ? (
              <>
  ...
  return (
    <Form name="power_form" layout="vertical" form={form}>
      <Table columns={columns} dataSource={data} pagination={false} bordered title={() => "电源输入特性"} />
    </Form>
  );
};
